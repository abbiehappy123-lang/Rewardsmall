...
- 资产：`assets/images/hero-bg.png`（Figma: `img1`）
- z-index: 0，铺满宽度
...
- 资产：`assets/images/badge-free.png`（Figma: `img5`）
- **z-index: 2，必须在编组24之上**

#### 5. 编组24 活动时间卡（top:188px left:12px w:351px h:77px）
- 资产：`assets/images/meta-card-bg.png`（Figma node `33:324`，`img241`）
- z-index: 1（低于奖励标识）
...
- **标题区（h:52px）**：
  - 底纹图：`assets/images/title-bg.png`，`bottom:8px 居中 w:152px h:15px`
...
- **权益列表（top:65px left:16px，flex column gap:8px）**：
  ...
  - 权益1图标：`assets/images/icon-benefit-3.png`（img21）→ 乘客实付金额100%<橙>给到司机
  - 权益2图标：`assets/images/icon-benefit-2.png`（img11）→ 免佣保底: <橙>免佣金额≥劳动报酬10%
  - 权益3图标：`assets/images/icon-benefit-1.png`（img4）→ 亏必补！没回本即<橙>返</>购卡金额<橙>价差
...
- **地图区**：`w:319px h:158px border-radius:24px overflow:hidden background:#e8edf2`，固定切图 `assets/images/map-area.png`（编组_3），直接引用不手绘，图片本身也加 `border-radius:24px`
- **标签行**（订单类型/接单方式/顺路类型）：
...
- **时间轴**（`position:relative height:248px width:319px margin:0 auto`，绝对坐标）：
  ...
  - 时钟图标：`assets/images/icon-clock.png`（透明背景PNG切图），`width:17px height:17px display:block flex-shrink:0`
...
- **已结束·获奖**：插画 `assets/images/driver-thumb.png`，文案："恭喜您累计获得**4单免佣**，累计收益**30元**"（数字橙色 `#FF6435`）
- 分割线：`width:319px height:0.5px background:#EBECF2 margin-bottom:14.5px`
- 订单链接行：复用进行中样式
...
|------|---------|------------|------|
| 头图背景 | `assets/images/hero-bg.png` | `img1` | 头图渐变大图 |
| 活动时间卡背景 | `assets/images/meta-card-bg.png` | `img241` / `33:324` | 编组24切图 |
| 奖励标识 | `assets/images/badge-free.png` | `img5` | "免"字贴纸 |
| 权益标题底纹 | `assets/images/title-bg.png` | `img3` | 标题背景装饰 |
| 权益图标1（100%给到司机） | `assets/images/icon-benefit-3.png` | `img21` | 奖励详情-免-2 |
| 权益图标2（免佣保底） | `assets/images/icon-benefit-2.png` | `img11` | 奖励详情-免-1 |
| 权益图标3（亏必补） | `assets/images/icon-benefit-1.png` | `img4` | 奖励详情-免 |
| 司机插画（获奖）| `assets/images/driver-thumb.png` | — | 小滴竖大拇指 |
| 司机插画（未获奖）| `assets/images/driver-sad.png` | — | 分组12 |
| 时间轴节点（激活）| `assets/images/timeline-dot.png` | `imgGroup24` | 编组24节点，进行中时段 |
...
| 地图切图 | `assets/images/map-area.png` | `编组_3` | 活动区域地图，圆角24px |
| 字体 | `assets/fonts/MFYuanHei-Boldtalic.ttf` | — | 标题/数字专用字体 |
...
6. **活动说明卡**：标题与正文无间距（`gap:0`，`.desc-card .sec-title-row { margin-bottom:0 }`），底部 `padding:16px`
7. **地图切图**：`border-radius:24px`，容器和图片均设置，`overflow:hidden`，`background:#e8edf2`
8. **布局策略**：权益卡及以上用 `position:absolute`，活动要求卡和说明卡用文档流（`.flow-content { margin-top:535px gap:10px }`），phone `padding-bottom:105px` 撑开底部
...
11. **进度轴节点色值（必查表，勿凭记忆）**：生效中节点=外圈 `#FFE0D7` + 内圈 `#FF6435`；**未开始（未选中）节点=外圈 `#FFFFFF` + 内圈 `#FFCBBB`**。两者外圈颜色不同，切勿都用 `#FFE0D7`，否则未开始与生效中难以区分。
